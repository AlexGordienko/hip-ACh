	...
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		"Network": &params.Sheet{
			...
			{Sel: ".HippoCHL", Desc: "hippo CHL projections -- no norm, moment, but YES wtbal = sig better",
				Params: params.Params{
					"Prjn.CHL.Hebb":          "0.05",
					// "Prjn.CHL.Hebb":          String(0.05*bfInteg),
			...
			{Sel: "#CA3ToCA3", Desc: "CA3 recurrent cons",
				Params: params.Params{
					...
					"Prjn.WtScale.Rel": fmt.Sprintf("%f", cathree),
					"Prjn.Learn.Lrate": fmt.Sprintf("%f", cathree),
				}},
			...
			{Sel: ".EC", Desc: "all EC layers: only pools, no layer-level",
				Params: params.Params{
					...
					// "Layer.Inhib.Pool.Gi":     fmt.Sprintf("%f", 2.0*bfInteg),
			...
			{Sel: "#DG", Desc: "very sparse = high inibhition",
				Params: params.Params{
					...
					// "Layer.Inhib.Layer.Gi":    fmt.Sprintf("%f", 3.8*bfInteg),
				}},
			{Sel: "#CA3", Desc: "sparse = high inibhition",
				Params: params.Params{
					"Layer.Inhib.ActAvg.Init": "0.02",
					// "Layer.Inhib.Layer.Gi":    "2.8",
					"Layer.Inhib.Layer.Gi":    fmt.Sprintf("%f", 2.8*bfInteg),
				}},
			{Sel: "#CA1", Desc: "CA1 only Pools",
				Params: params.Params{
					...
					// "Layer.Inhib.Pool.Gi":     fmt.Sprintf("%f", 2.4*bfInteg),
					"Layer.Inhib.Pool.On":     "true",
				}},
		},
	}},
...
func (ss *Sim) AlphaCyc(train bool) {
	...
	dg := ss.Net.LayerByName("DG").(leabra.LeabraLayer).AsLeabra() // added
	...
	// Logging values initialized for cycle data
	...
	// Logging of unit activation values for analysis

	if (!train) {
		//t := time.Now()
		//tfor := t.Format("2006_01_02_0304")
		fmt.Println(ss.TrainEnv.TrialName.Cur)
		dirpathacts := ("output/")
		...
		filew, _ := os.OpenFile("./" + dirpathacts+ "session_20percent" + ".csv", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
		defer filew.Close()
		writerw := csv.NewWriter(filew)
		defer writerw.Flush()

		
		if ss.headersBool == false { //need to change - only run this once.
		...
		// Select which cycle to record
		trialNum := 99

		valueStr = []string{fmt.Sprint(ss.TrainEnv.Run.Cur), fmt.Sprint(ss.TrainEnv.Epoch.Cur), fmt.Sprint(trialNum), fmt.Sprint(ss.TestEnv.TrialName.Cur)}
		for _, vals := range ecinTrlCycActs[trialNum] {
			valueStr = append(valueStr, fmt.Sprint(vals))
		}
		for _, vals := range ecoutTrlCycActs[trialNum] {
			valueStr = append(valueStr, fmt.Sprint(vals))
		}
		for _, vals := range dgTrlCycActs[trialNum] {
			valueStr = append(valueStr, fmt.Sprint(vals))
		}
		for _, vals := range ca3TrlCycActs[trialNum] {
			valueStr = append(valueStr, fmt.Sprint(vals))
		}
		for _, vals := range ca1TrlCycActs[trialNum] {
			valueStr = append(valueStr, fmt.Sprint(vals))
		}
		writerw.Write(valueStr)
...
func (ss *Sim) OpenPats() {
	...
	// pattern separation
	ss.OpenPatAsset(ss.TrainAB, "train_ab_ps.tsv", "TrainABPS", "AB Training Patterns PS")
	ss.OpenPatAsset(ss.TrainAC, "train_ac.tsv", "TrainAC", "AC Training Patterns")
	ss.OpenPatAsset(ss.TestAB, "test_ab_ps.tsv", "TestABPS", "AB Testing Patterns PS")
	ss.OpenPatAsset(ss.TestAC, "test_ac.tsv", "TestAC", "AC Testing Patterns")
	ss.OpenPatAsset(ss.TestLure, "test_lure_ps.tsv", "TestLurePS", "Lure Testing Patterns PS")
}
...
func (ss *Sim) CmdArgs() {
	...
	flag.IntVar(&ss.MaxEpcs, "epcs", 20, "maximum number of epochs to run (split between AB / AC)")
